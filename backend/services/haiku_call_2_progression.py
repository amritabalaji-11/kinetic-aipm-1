...
async def run_haiku_call_2(analysis_id: str) -> None:
    ...
    async with HAIKU_CALL_2_SEMAPHORE:
        ...
        # Small delay so Haiku Call 1 clears the TPM window
            ...
            if not previous:
                ...
                await db.execute(
                    ...
                    INSERT OR IGNORE INTO progression_results (
                        analysis_id, session_id, user_id, available, error_code
                    )
                    VALUES (:aid, :sid, :uid, 0, 'NO_PREVIOUS_SESSION')
                    """,
...
{{
  ...
  "movement_quality_trend": "up | down | stable"
            ...
            # ── Update form_analysis_results with haiku_call_2 output ─────
            await db.execute(
                """
            ...
            try:
                ...
                print(f"[Haiku Call 2] Failed sending SSE: {sse_error}")

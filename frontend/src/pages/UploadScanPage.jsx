...
function UploadScanPage() {
  ...
  function handleFileChange(e) {
    ...
    if (fileSizeMB > MAX_FILE_SIZE_MB) {
      setFileError(`File too large (${fileSizeMB.toFixed(1)}MB). Max ${MAX_FILE_SIZE_MB}MB.`)
      setVideoFile(null)
      setVideoPreviewUrl(null)
      return
    }
    ...
    <div className="min-h-screen bg-light-bg text-text-primary p-4">
      <div className="max-w-md mx-auto space-y-4">
        ...
        <section className="border-2 border-dashed border-cyan-glow/60 rounded-xl p-4 bg-light-card shadow-sm">
          ...
          {/* Anatomy diagram with static muscle highlight */}
          <div className="relative flex justify-center mb-6">
            <div className="relative w-64 h-84">
              ...
              {/* Muscle labels — dark text on light background */}
            ...
            {/* SVG overlay for label connector lines */}
            <svg
        ...
        <section className="border-2 border-dashed border-cyan-glow/60 rounded-xl p-4 bg-light-card shadow-sm">
          ...
          >
            ...
            <span className="text-error text-lg leading-none">
              {tipsExpanded ? "—" : "+"}
            </span>
          </button>
          ...
          {tipsExpanded && (
            <>
              ...
              <ul className="text-xs text-text-secondary space-y-1.5 list-disc list-inside leading-relaxed">
                ...
                <li><strong>Distance</strong>: Stand about 6–8 feet away so your entire body remains in the frame at the bottom of the squat.</li>
        ...
        <section>
          ...
          {videoPreviewUrl ? (
            <div className="relative rounded-xl overflow-hidden bg-light-card border border-gray-200 shadow-sm">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                <button
                  type="button"
                  onClick={(e) => {
                    const vid = e.currentTarget.parentElement.previousElementSibling
            ...
            <div className="border-2 border-dashed border-cyan-glow/60 rounded-xl p-6 bg-light-card text-center shadow-sm">
              ...
              <p className="text-xs text-text-teritary mb-4">Supports high-quality files.</p>
              <button
                type="button"
                onClick={() => document.getElementById("video-upload").click()}
        ...
        <section className="border-2 border-dashed border-cyan-glow/60 rounded-xl p-4 bg-light-card shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <label className="text-xs uppercase tracking-wider text-text-teritary">
              Load Parameters
            </label>
            ...
            <div className="flex bg-gray-100 rounded-md p-0.5 text-xs">
              <button
                ...
                className={`px-2 py-0.5 rounded transition-colors ${
                  unit === "kg" ? "bg-cyan-glow text-text-primary font-semibold" : "text-text-teritary"
                }`}
              >
                KG
              </button>
              <button
                ...
                onClick={() => setUnit("lbs")}
                className={`px-2 py-0.5 rounded transition-colors ${
                  unit === "lbs" ? "bg-cyan-glow text-text-primary font-semibold" : "text-text-teritary"
                }`}
              >
                LBS
              </button>
